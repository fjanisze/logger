...
{
    SET_LOG_THREAD_NAME( "THREAD" );
    auto gen = bind( uniform_int_distribution<>{ 0 , 4 }, default_random_engine{} );
    for( long i = 0 ; i < amount ; i++ )
    {
        LOG1("heavily_logging_thread(): This is one (",i,") of ", amount, " prints, thread: ",this_thread::get_id(),", ", prints[ gen() ] );
    }
}
...
try{
    ...
    if( io_file )
    {
        ...
        for( short i = 0 ; i < exec_loop ; i++ )
        {
            ...
            for( long i = 0 ; i < amount ; i++ )
            {
                LOG1("main(): This is one (",i,") of ", amount, " prints that are going to be printed by the thread ",this_thread::get_id() );
            }
...
{
    ERR("main(): Exception caught at this point, this is fatal!");
}
